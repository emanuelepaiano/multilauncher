...
startVm() {
  url=${START_VM_API/VMID/$VMID}
  url=${url/NODE/$NODE}
  COMMAND="$COMMAND $url"
  (echo -n "Starting VM $VMID on node $NODE.." &&
  echo "" &&
  eval "$COMMAND" &&
  echo ""
...
stopVm() {
  url=${STOP_VM_API/VMID/$VMID}
  url=${url/NODE/$NODE}
  COMMAND="$COMMAND $url"
  (echo -n "Stopping VM $VMID on node $NODE.." &&
  echo "" &&
  eval "$COMMAND" &&
  echo ""
  echo "DONE") || (echo "ERROR" && exit 1)
...
shutdownVm() {
  url=${SHUTDOWN_VM_API/VMID/$VMID}
  url=${url/NODE/$NODE}
  COMMAND="$COMMAND $url"
  (echo -n "Shutdown VM $VMID on node $NODE.." &&
  echo "" &&
  eval "$COMMAND" &&
  echo ""
  echo "DONE") || (echo "ERROR" && exit 1)
...
resetVm() {
  url=${RESET_VM_API/VMID/$VMID}
  url=${url/NODE/$NODE}
  COMMAND="$COMMAND $url"
  (echo -n "Forcing reboot VM $VMID on NODE $NODE.." &&
  echo "" &&
  eval "$COMMAND" &&
  echo ""
  echo "DONE") || (echo "ERROR" && exit 1)
